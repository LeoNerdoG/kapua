...
public class ConfigurationPrinter {
    ...
    public void printLog() {
        ...
        //
        // Title
        String alignedTitle = buildAlignedTitle();
        printLogLeveled(alignedTitle);

        //
        // Parameters
        for (Configuration configuration : getConfigurations()) {
            printLogLeveled("|  ".concat(configuration.toString()));
        }

        //
        // End Line - Same length of Title
        String footerLog = new String(new char[alignedTitle.length()]).replace('\0', '=');
        printLogLeveled(footerLog);
    }

    /**
     * Aligns the {@link #getTitle()} according to {@link #getTitleAlignment()}
     *
     * @return The {@link #getTitle()} aligned.
     * @since 1.3.0
     */
    private String buildAlignedTitle() {
        StringBuilder stringBuilder = new StringBuilder();
        switch (getTitleAlignment()) {
            case LEFT: {
                stringBuilder.append("= ").append(getTitle()).append(" =====================================");
            }
            break;
            case CENTER: {
                stringBuilder.append("=================== ").append(getTitle()).append(" ===================");
    ...
    private void printLogLeveled(String log) {
        switch (getLogLevel()) {
            case DEBUG:
                getParentLogger().debug(log);
                break;
            case ERROR:
                getParentLogger().error(log);
                break;
            case INFO:
                getParentLogger().info(log);
                break;
            case TRACE:
                getParentLogger().trace(log);
                break;
            case WARN:
                getParentLogger().warn(log);
                break;
        }
    }
